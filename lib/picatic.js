...
function Picatic(options) {
    ...
    let authentication_options = {headers:{}};

    // v2 use OAuth 2.0
    if (this.options.api_version === 'v2')
        authentication_options.headers = {
            "Authorization": 'Bearer ' + this.options.access_key
        }
    else // use special access key
        authentication_options.headers = {
...
Picatic.prototype.__buildEndpoint = function (path) {

    var endpoint = this.options.rest_base + '/' + this.options.api_version;
